...
exports.mask_right1 = exports.mask_left1 = undefined;
...
function _makeSlots(startOffset, date, bitset, duration, resId, taxId, vectorSlotSize) {
  ...
  let freeSlots = (0, _vector.getCRACFreeSlots)(Math.floor(startOffset / vectorSlotSize), bitset, Math.floor(30 / vectorSlotSize));
...
function makeSlots(startOffset, slots, taxonomyId, duration, vectorSlotSize) {
  return slots.reduce(function (ret, day) {
    ...
    resources.forEach(function (r) {
      var bs = (0, _vector.prepareBitset)(r.bitset, vectorSlotSize);
...
function calculateWorkloadWeights(slots, vectorSlotSize) {
  return Object.values((slots || []).reduce(function (ret, day) {
    ...
    resources.forEach(function (r) {
      let bs;
      try {
        bs = (0, _vector.prepareBitset)(r.bitset, vectorSlotSize);
      } catch (e) {
 ...
 * @type {{}}
 */
const mask_right1 = exports.mask_right1 = [0, 1, 3, 7, 15, 31, 63, 127, 255, 511, 1023, 2047, 4095, 8191, 16383, 32767, 65535, 131071, 262143, 524287, 1048575, 2097151, 4194303, 8388607, 16777215, 33554431, 67108863, 134217727, 268435455, 536870911, 1073741823, 2147483647, 4294967295];
...
function buildBookingCRACVector(bitset, offset, sz, vectorSlotSize) {
  let r = (0, _vector.newBusyBitset)(vectorSlotSize);
  let p = {};
 ...
 */
function buildSequenceBookingCRACVector(bookingBitSets, durations, vectorSlotSize) {
  let leftShift = 0;
  let bookingVector = (0, _vector.newFreeBitset)(vectorSlotSize);
  for (let i = 0; i < bookingBitSets.length; i++) {
    ...
    bookingVector = (0, _vector.setAnd)(bookingVector, curBookingVector);
...
function printCRACVector(bitset, int32delimiter = '.') {
